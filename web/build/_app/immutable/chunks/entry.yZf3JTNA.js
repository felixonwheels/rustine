import { n as J, aG as ut, aF as dt } from './runtime.DilaVa7n.js';
function ht(e, n, t) {
	if (e == null) return n(void 0), J;
...
const _t = ['href', 'pathname', 'search', 'toString', 'toJSON'];
function yt(e, n, t) {
	const r = new URL(e);
...
const wt = '/__data.json',
	vt = '.html__data.json';
function bt(e) {
	return e.endsWith('.html') ? e.replace(/\.html$/, vt) : e.replace(/\/$/, '') + wt;
}
function kt(...e) {
	let n = 5381;
	for (const t of e)
...
function St(e, n) {
	...
	if (r != null && r.textContent) {
		...
		return (
			i && M.set(t, { body: a, init: o, ttl: 1e3 * Number(i) }),
...
function Ut({ nodes: e, server_loads: n, dictionary: t, matchers: r }) {
	const a = new Set(n);
	return Object.entries(t).map(([s, [c, l, u]]) => {
		const { pattern: h, params: g } = It(s),
			d = {
				id: s,
				exec: (_) => {
					const f = h.exec(_);
					if (f) return Tt(f, g, r);
				},
...
function _e(e, n) {
	...
	const r = e instanceof SVGAElement ? e.target.baseVal : e.target,
		a = !t || !!r || oe(t, n) || (e.getAttribute('rel') || '').split(/\s+/).includes('external'),
...
const Bt = 'x-sveltekit-invalidated',
	Kt = 'x-sveltekit-trailing-slash';
function Q(e) {
	return e instanceof se || e instanceof Ee ? e.status : 500;
}
function zt(e) {
...
const N = He(Ke) ?? {},
	K = He(Be) ?? {},
	U = { url: je({}), page: je({}), navigating: be(null), updated: jt() };
function Se(e) {
	N[e] = ke();
}
function Yt(e, n) {
...
async function cn(e, n, t) {
	...
	document.URL !== location.href && (location.href = location.href),
		...
		(L = (o = history.state) == null ? void 0 : o[B]),
		S || ((S = L = Date.now()), history.replaceState({ ...history.state, [D]: S, [B]: L }, ''));
	const r = N[S];
	r && ((history.scrollRestoration = 'manual'), scrollTo(r.x, r.y)),
		t ? await rn(we, t) : tn(location.href, { replaceState: !0 }),
		nn();
}
async function Wt() {
	if ((await (ge || (ge = Promise.resolve())), !ge)) return;
	ge = null;
	const e = fe(y.url, !0);
	R = null;
	const n = (V = {}),
		t = e && (await Te(e));
	if (!(!t || n !== V)) {
		if (t.type === 'redirect') return Ie(new URL(t.location, y.url).href, {}, 1, n);
		t.props.page && (I = t.props.page), (y = t.state), tt(), ce.$set(t.props);
	}
}
function tt() {
	(te.length = 0), (Re = !1);
...
function nt(e) {
	ne.some((n) => (n == null ? void 0 : n.snapshot)) &&
		(K[e] = ne.map((n) => {
			var t;
...
function rt(e) {
	...
	(n = K[e]) == null ||
		n.forEach((t, r) => {
			var a, o;
			(o = (a = ne[r]) == null ? void 0 : a.snapshot) == null || o.restore(t);
		});
}
function Ce() {
	Se(S), Ne(Ke, N), nt(L), Ne(Be, K);
}
		...
		H.add(n),
			(R = {
				...
				promise: Te({ ...e, preload: n }).then(
					(t) => (H.delete(n), t.type === 'loaded' && t.state.error && (R = null), t)
				)
			});
	}
	return R.promise;
...
function at(e, n, t) {
	...
	r && r.remove(),
		...
		(ce = new q.root({
			target: n,
			props: { ...e.props, stores: U, components: ne },
			hydrate: t,
			sync: !1
		})),
		rt(L);
...
function ae({ url: e, params: n, branch: t, status: r, error: a, route: o, form: i }) {
	let s = 'never';
	if (P && (e.pathname === P || e.pathname === P + '/')) s = 'always';
	else for (const d of t) (d == null ? void 0 : d.slash) !== void 0 && (s = d.slash);
	(e.pathname = pt(e.pathname, s)), (e.search = e.search);
	const c = {
		type: 'loaded',
		state: { url: e, params: n, branch: t, error: a, route: o },
		props: { constructors: Ht(t).map((d) => d.node.component), page: I }
	};
...
async function Le({ loader: e, parent: n, url: t, params: r, route: a, server_data_node: o }) {
	var u, h, g;
	let i = null,
		s = !0;
	const c = {
...
function Ve({ error: e, url: n, route: t, params: r }) {
	return {
		type: 'loaded',
...
async function Te({ id: e, invalidating: n, url: t, params: r, route: a, preload: o }) {
	if ((R == null ? void 0 : R.id) === e) return H.delete(R.token), R.promise;
		...
		const k = y.branch[v],
			E =
				...
				((k == null ? void 0 : k.loader) !== p[1] ||
					Fe(_, g, h, d, (x = k.server) == null ? void 0 : x.uses, r));
		return E && (_ = !0), E;
	});
	if (f.some(Boolean)) {
		try {
			...
			return H.has(o)
				? Ve({ error: v, url: t, params: r, route: a })
				: le({ status: Q(p), error: v, url: t, route: a });
		}
	...
	const A = l.map(async (p, v) => {
		...
		if (
			...
			!Fe(b, g, h, d, (ue = k.universal) == null ? void 0 : ue.uses, r)
		)
			return k;
		...
		return Le({
			...
			parent: async () => {
				...
				for (let de = 0; de < v; de += 1)
					Object.assign(Ue, (xe = await A[de]) == null ? void 0 : xe.data);
				return Ue;
			},
			server_data_node: Pe(
	...
	for (let p = 0; p < l.length; p += 1)
		if (l[p])
			try {
				...
				else if (v instanceof se) E = v.body;
				else {
					if (await U.updated.check()) return await F(t);
					E = await C(v, { params: r, url: t, route: { id: a.id } });
				}
				const x = await Qt(p, w, i);
				return x
					? ae({
							url: t,
							params: r,
...
async function le({ status: e, error: n, url: t, route: r }) {
	const a = {};
	let o = null;
	...
	return ae({ url: t, params: a, branch: [s, c], status: e, error: n, route: null });
}
function fe(e, n) {
	if (!e || oe(e, P)) return;
	let t;
	...
	for (const a of ie) {
		const o = a.exec(r);
		if (o) return { id: e.pathname + e.search, invalidating: n, route: a, params: mt(o), url: e };
	}
}
function ot(e) {
	return gt(e.slice(P.length) || '/');
}
function st({ url: e, type: n, intent: t, delta: r }) {
	...
	return z || Qe.forEach((s) => s(i)), a ? null : o;
}
async function W({
	type: e,
	url: n,
	...
	const h = fe(n, !1),
		g = st({ url: n, type: e, delta: t == null ? void 0 : t.delta, intent: h });
	if (!g) {
		u();
		return;
	}
	const d = S,
		_ = L;
	l(), (z = !0), re && U.navigating.set(g.navigation), (V = c);
	let f = h && (await Te(h));
	if (!f) {
		if (oe(n, P)) return await F(n);
		f = await it(
			n,
			{ id: null },
			await C(new Ee(404, 'Not Found', `Not found: ${n.pathname}`), {
				url: n,
				params: {},
				route: { id: null }
			}),
	...
	if (f.type === 'redirect')
		...
		else return Ie(new URL(f.location, n).href, {}, s + 1, c), !1;
	...
	if (
		...
		const w = o ? 0 : 1,
			p = { [D]: (S += w), [B]: (L += w), [ze]: i };
		(o ? history.replaceState : history.pushState).call(history, p, '', n), o || Yt(S, L);
	}
	if (((R = null), (f.props.page.state = i), re)) {
		(y = f.state), f.props.page && (f.props.page.url = n);
		const w = (await Promise.all(Jt.map((p) => p(g.navigation)))).filter(
			(p) => typeof p == 'function'
		);
		if (w.length > 0) {
			let p = function () {
...
async function it(e, n, t, r) {
	return e.origin === Y && e.pathname === location.pathname && !Ae
		? await le({ status: r, error: t, url: e, route: n })
		: await F(e);
}
function en() {
	let e;
	...
	const t = new IntersectionObserver(
		(o) => {
			for (const i of o) i.isIntersecting && (me(i.target.href), t.unobserve(i.target));
		},
		{ threshold: 0 }
	);
	function r(o, i) {
		const s = We(o, T);
		if (!s) return;
		const { url: c, external: l, download: u } = _e(s, P);
		if (l || u) return;
		const h = Z(s);
		if (!h.reload)
			if (i <= h.preload_data) {
				const g = fe(c, !1);
				g && Xt(g);
			} else i <= h.preload_code && me(c.pathname);
	}
	function a() {
		t.disconnect();
		for (const o of T.querySelectorAll('a')) {
			const { url: i, external: s, download: c } = _e(o, P);
			if (s || c) continue;
			const l = Z(o);
			l.reload ||
				(l.preload_code === X.viewport && t.observe(o),
				l.preload_code === X.eager && me(i.pathname));
		}
	}
	$.push(a), a();
}
function C(e, n) {
	...
	const t = Q(e),
		r = zt(e);
	return q.hooks.handleError({ error: e, event: n, status: t, message: r }) ?? { message: r };
}
function tn(e, n = {}) {
	return (e = Ye(e)), e.origin !== Y ? Promise.reject(new Error('goto: invalid URL')) : Ie(e, n, 0);
}
function ln(e) {
...
function nn() {
	...
	(history.scrollRestoration = 'manual'),
		addEventListener('beforeunload', (t) => {
			let r = !1;
			if ((Ce(), !z)) {
				const a = ft(y, void 0, null, 'leave'),
					o = {
						...a.navigation,
						cancel: () => {
		...
		((n = navigator.connection) != null && n.saveData) || en(),
		T.addEventListener('click', async (t) => {
			...
			if (
				...
				return;
			const r = We(t.composedPath()[0], T);
			if (!r) return;
			const { url: a, external: o, target: i, download: s } = _e(r, P);
			if (!a) return;
			if (i === '_parent' || i === '_top') {
			...
			if (
				...
				return;
			if (o || c.reload) {
				st({ url: a, type: 'link' }) ? (z = !0) : t.preventDefault();
				return;
			}
			...
			if (h !== void 0 && u === he(location)) {
				...
				if (((G = !0), Se(S), e(a), !c.replace_state)) return;
				G = !1;
			}
			t.preventDefault(),
				await new Promise((d) => {
			...
			if ((r = t.state) != null && r[D]) {
				...
				if (c === L && (et || l)) {
					e(s),
						(N[S] = ke()),
						o && scrollTo(o.x, o.y),
						i !== I.state && ((I = { ...I, state: i }), ce.$set({ page: I })),
						(S = a);
					return;
				}
				const h = a - S;
		...
		addEventListener('hashchange', () => {
			G &&
				((G = !1), history.replaceState({ ...history.state, [D]: ++S, [B]: L }, '', location.href));
		});
...
async function rn(
	...
	({ params: a = {}, route: o = { id: null } } = fe(c, !1) || {});
	let l;
	try {
		const u = r.map(async (d, _) => {
			...
			g = ie.find(({ id: d }) => d === o.id);
		if (g) {
			const d = g.layouts;
			for (let _ = 0; _ < d.length; _++) d[_] || h.splice(_, 0, void 0);
		}
		l = ae({ url: c, params: a, branch: h, status: n, error: t, form: s, route: g ?? null });
	} catch (u) {
		if (u instanceof Ze) {
	...
	l.props.page && (l.props.page.state = {}), at(l, e, !0);
}
async function ct(e, n) {
	var a;
	const t = new URL(e);
	(t.pathname = bt(e.pathname)),
		e.pathname.endsWith('/') && t.searchParams.append(Kt, '1'),
		t.searchParams.append(Bt, n.map((o) => (o ? '1' : '0')).join(''));
	const r = await Me(t.href);
	if (!r.ok) {
		let o;
		throw (
...
export { U as a, P as b, cn as c, sn as g, ln as i, ht as s };
